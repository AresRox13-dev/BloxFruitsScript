local function createToggle(parent, name, configKey, callback)
        ...
        btn.MouseButton1Click:Connect(function()
            ...
            Services.TweenService:Create(btn, TweenInfo.new(0.2), {
                BackgroundColor3 = Config[configKey] and Color3.fromRGB(50, 200, 100) or Color3.fromRGB(180, 50, 50)
            }):Play()
    ...
    log("UI initialized", Color3.fromRGB(100, 255, 100))
end
...
local function startMainLoops()
    ...
    log("All systems operational", Color3.fromRGB(100, 255, 100))
end

task.spawn(function()
    log("╔════════════════════════════════════════╗", Color3.fromRGB(100, 200, 255))
    log("║  Blox Fruits Pen Test Suite v3.5     ║", Color3.fromRGB(100, 200, 255))
    log("║  Enhanced & Optimized Edition         ║", Color3.fromRGB(100, 200, 255))
    log("╚════════════════════════════════════════╝", Color3.fromRGB(100, 200, 255))
    ...
    if Config.ESPBosses then
        for _, boss in pairs(State.Cache.Bosses) do
            if boss.Root and boss.Root.Parent and boss.Humanoid.Health > 0 then
                ...
                if Config.ESPDistance then
                    text = text .. "\n[" .. dist .. "m] HP:" .. math.floor(boss.Humanoid.Health)
                end
    ...
    pcall(function()
        ...
        mt.__namecall = newcclosure(function(self, ...)
            ...
            if Config.AntiKick and method == "Kick" then
                log("Blocked kick attempt", Color3.fromRGB(255, 100, 100))
                return
            end
...
local function createNotification(title, message, duration)
    ...
    Services.TweenService:Create(notif, TweenInfo.new(0.4, Enum.EasingStyle.Back, Enum.EasingDirection.Out), {
        Position = UDim2.new(1, -330, 1, -100 - (#notifications * 100))
    }):Play()
...
local function createPasswordScreen()
    ...
    local function checkPassword()
        if textbox.Text == Config.Password then
            ...
            Services.TweenService:Create(frame, TweenInfo.new(0.3), {
            ...
            for i = 1, 4 do
                Services.TweenService:Create(frame, TweenInfo.new(0.08), {
                    Position = originalPos + UDim2.new(0, 12 * (i % 2 == 0 and -1 or 1), 0, 0)
                }):Play()
                task.wait(0.08)
            end
...
function initializeUI()
    ...
    local function createToggle(parent, name, configKey, callback)
        ...
        local label = Instance.new("TextLabel")
        label.Size = UDim2.new(1, -85, 1, 0)
        label.Position = UDim2.new-- Blox Fruits Advanced Penetration Testing Suite v3.5
...
local GameData = {
    ...
    Locations = workspace:FindFirstChild("_WorldOrigin") and workspace._WorldOrigin:FindFirstChild("Locations"),
    CombatRemote = Services.ReplicatedStorage:FindFirstChild("Remotes") and Services.ReplicatedStorage.Remotes:FindFirstChild("CommF_"),
...
-- ===============================================
-- ENHANCED HAKI SYSTEM (FIXED)
-- ===============================================

local function enableHaki()
    if not Config.AutoHaki then return end
    
    -- Only toggle every 5 seconds to prevent rapid on/off
    if tick() - State.Combat.LastHakiToggle < 5 then
        return
    end
...
local function enableObservation()
    ...
    -- Only toggle every 5 seconds
    if tick() - State.Combat.LastObsToggle < 5 then
        return
    end
...
local function smoothTeleport(targetCFrame, speed)
    if State.Active.Teleporting then
        return false
    end
    
    if tick() - lastTeleportTime < 0.2 then
        return false
    end
    ...
    if Config.SafeMode and distance > 15000 then
        log("Teleport distance too large: " .. math.floor(distance), Color3.fromRGB(255, 100, 100))
        State.Active.Teleporting = false
        return false
    end
    ...
    while getMagnitude(root.Position, targetCFrame.Position) > 3 do
        ...
        -- Maintain height during long distance travel
        ...
        -- Keep velocity at zero to prevent falling
...
local function scanChests()
    ...
    log("Found " .. #State.Cache.Chests .. " chests", Color3.fromRGB(255, 255, 0))
end
...
local function scanFruits()
    ...
    log("Found " .. #State.Cache.Fruits .. " fruits", Color3.fromRGB(255, 0, 255))
end
...
local function scanBosses()
    ...
    log("Found " .. #State.Cache.Bosses .. " bosses", Color3.fromRGB(255, 0, 0))
end
...
local function performAttack()
    ...
    local now = tick()
    if now - lastAttackTime < (Config.FastAttack and 0.05 or attackCooldown) then
        return false
    end
...
local function attackEnemy(enemy)
    if not enemy or not enemy.Humanoid or enemy.Humanoid.Health <= 0 then
        return false
    end
